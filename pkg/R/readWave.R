...
function(filename, from = 1, to = Inf, 
    ...
    RIFF <- readChar(con, 4)
    file.length <- readBin(con, int, n = 4, size = 1, endian = "little", signed=FALSE) %*% 2^c(0,8,16,24)
    WAVE <- readChar(con, 4)
    ...
    if (header && (tolower(FMT) == "bext")){
        bext.length <- readBin(con, int, n = 4, size = 1, endian = "little", signed=FALSE) %*% 2^c(0,8,16,24)
    ...
    while(FMT != "fmt "){
        i <- i+1
        belength <- readBin(con, int, n = 4, size = 1, endian = "little", signed=FALSE) %*% 2^c(0,8,16,24)
        seek(con, where = belength, origin = "current")
        FMT <- readChar(con, 4)
        if(i > 5) stop("There seems to be no 'fmt ' chunk in this Wave (?) file.")
    }
    fmt.length <- readBin(con, int, n = 4, size = 1, endian = "little", signed=FALSE) %*% 2^c(0,8,16,24)
...
#    }
    ...
    data.length <- readBin(con, int, n = 4, size = 1, endian = "little", signed=FALSE) %*% 2^c(0,8,16,24)
    bytes <- bits/8
